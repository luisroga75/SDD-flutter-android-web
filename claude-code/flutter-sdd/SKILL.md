---
name: flutter-sdd
description: Guía el desarrollo SDD de aplicaciones Flutter y Dart en Claude Code, desde la entrevista hasta specs, planes, tareas, implementación y validación con Git/GitHub. Úsala para iniciar o continuar ese ciclo, no para preguntas aisladas de Dart.
---

# Flutter SDD para Claude Code

Invocación: `/flutter-sdd` seguida del objetivo. Trabaja en el proyecto del usuario, no en esta carpeta. Lee los CLAUDE.md e instrucciones importadas aplicables y la constitución. En un proyecto nuevo de esta práctica propone `AGENTS.md` común y `CLAUDE.md` con `@AGENTS.md`, como el material original. Si ya usa solo CLAUDE.md o el usuario lo prefiere, conserva esa estructura. No presupongas que Claude lee AGENTS.md sin importación. No presupongas herramientas de Codex ni un conector GitHub.
...
Compara `flutter doctor -v` con los destinos acordados. Una alerta de Xcode en un flujo solo Android no bloquea la redacción. Nunca pruebes iOS localmente en Windows/Linux ni declares validada una plataforma no ejecutada. No actualices SDK, sistema operativo ni paquetes fuera del encargo. No actives omisión de permisos ni incluyas comandos con ejecución automática en el frontmatter.
